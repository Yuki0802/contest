...
from tool.utils import get_eer, get_lr, change_lr
...
def main():
    ...
    if opt.conv_type == "1D":
        model = getattr(model_1d, opt.model)(
            in_dim=opt.in_planes, embedding_size=opt.embd_dim, hidden_dim=opt.hidden_dim
    ...
    classifier = getattr(classifiers, opt.classifier)(
        opt.embd_dim,
        len(opt.spk2int) * 3 if opt.spk_aug else len(opt.spk2int),
    ...
    def lr_lambda(x):
        return opt.lr / (batch_per_epoch * opt.warm_up_epoch) * (x + 1)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (feats, key) in enumerate(train_loader):
            ...
            if dist.get_rank() == 0:
                writer.add_scalar(tag="train/loss", step=epoch,
                                  value=loss.data.item())
                writer.add_scalar(tag="train/acc", step=epoch, value=acc * 100)
                writer.add_scalar(tag="train/lr", step=epoch,
                                  value=get_lr(optimizer))
        ...
        if dist.get_rank() == 0:
            save_model(
                "EXP/%s" % opt.save_dir, epoch, model, classifier, optimizer, scheduler
            )
