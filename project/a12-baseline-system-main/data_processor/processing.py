...
def addspeed(speech, speed_value="1"):
    effects = [["speed", speed_value], ["rate", "16000"]]
    signal_sox, sample_rate = torchaudio.sox_effects.apply_effects_tensor(
        speech.unsqueeze(0), 16000, effects, channels_first=True
    )
    return signal_sox[0]
...
def addnoise(speech, noise_list, snr_dur=[0, 20], max_frames=200):
    ...
    noise = truncate_speech(
        load_wav(random.choice(noise_list[noies_type])),
...
def addreverberate(speech, noise_list, max_frames):
    ...
    signal_rir = scisignal.convolve(speech, rir_raw, mode="full")[
        : max_frames * 160 - 160
    ]
...
def _get_sample(path, resample=None):
    ...
    if resample:
        ...
        # https://github.com/pytorch/audio/issues/2691
        return torchaudio.sox_effects.apply_effects_tensor(
            torchaudio.load(path)[0], torchaudio.load(path)[1], effects=effects
        )
...
def load_wav(path, max_frames=200, fs=16000, train_mode=False):
    # FIXME on Windows
    signal, fs = _get_sample(path, resample=fs if fs != 16000 else None)
    ...
    if train_mode:
        ...
        if signalsize <= max_audio:
            ...
            startframe = np.array(
                [np.int64(random.random() * (signalsize - max_audio))]
            )[0]
...
def augment_spec(feat, opt):
    ...
    # https://github.com/DemisEom/SpecAugment/issues/24
