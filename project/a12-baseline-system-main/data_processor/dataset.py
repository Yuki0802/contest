...
from data_processor.processing import *
...
class WavDataset(Dataset):
    ...
    def __init__(self, opt=None, train_mode=True):
        ...
        if train_mode:
            ...
            self.utt2spk = {
                line.split()[0]: opt.spk2int[line.split()[1]]
                for line in open("DATA/%s/utt2spk" % opt.train_dir)
            }

            print(len(self.utt2spk))
    ...
    def __getitem__(self, idx):
        ...
        signal = load_wav(
            filename, max_frames=self.max_frames, fs=self.fs, train_mode=self.train_mode
        )
        if self.train_mode:
            if self.spk_aug:
            ...
            if self.data_wavaug:
                ...
                aug_type = random.sample(choices, 1)[0]
                if aug_type == "noise":
                    signal = addnoise(
